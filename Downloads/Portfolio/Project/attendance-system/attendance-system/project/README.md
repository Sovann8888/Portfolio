...
| Clickable Session History | Click any past session ("Session 1", "Session 2", ...) to expand it in place and see every student who checked in — Student ID, name, major, time, and device ID |
...
4. The QR auto-refreshes every 10 seconds; the live student count updates
   every 5 seconds.
5. Click **End Session**, then click that session's row in **Attendance
   Session History** to expand it in place and see every student who
   checked in (ID, name, major, time, device) — or use **CSV** to export it,
   or scroll down to see the full attendance history for every student ever
...
|---|---|---|
| GET/POST | `/login` | Step 1: enter email, get a code emailed |
| GET/POST | `/register` | Self-register a new professor account |
| GET/POST | `/login/verify` | Step 2: enter the 6-digit code |
| POST | `/api/login-code/resend` | Issue and (re)send a fresh code |
| GET | `/logout` | Clear session |

### Dashboard & Subjects
| Method | Route | Purpose |
|---|---|---|
| GET | `/dashboard` | List all subjects with student/session counts |
| POST | `/api/subjects/add` | Create a new subject `{name}` |
| GET | `/subject/<id>` | Subject detail: start session, session history, student roster |

### QR Sessions
| Method | Route | Purpose |
|---|---|---|
| POST | `/session/start` | Start a new session for a subject `{subject_id}` |
| POST | `/session/<sess_id>/end` | End an active session |
| GET | `/session/<sess_id>/qr` | Full-screen live QR display for the classroom |
| GET | `/api/qr/<sess_id>` | Current QR image + token (auto-rotates every 10s) |
| GET | `/api/session/<sess_id>/count` | Live count of students checked in so far |
| GET | `/api/session/<sess_id>/records` | JSON list of every check-in for a session — powers the clickable session-history accordion |
| GET | `/session/<sess_id>/records` | Standalone HTML records page (linked from CSV/direct access) |
| GET | `/session/<sess_id>/export` | Download that session's attendance as CSV |

### Student Check-In
| Method | Route | Purpose |
|---|---|---|
| GET | `/attend/<sess_id>/<token>` | Page a student lands on after scanning the QR |
| POST | `/api/attend/submit` | Submit `{session_id, token, student_id, device_id}` |

### Student Management
| Method | Route | Purpose |
|---|---|---|
| GET | `/students` | Manage-students page |
| POST | `/api/students/add` | Enroll a student `{student_id, name, major, subject_id}` |
| DELETE | `/api/students/delete/<student_id>` | Remove a student |
